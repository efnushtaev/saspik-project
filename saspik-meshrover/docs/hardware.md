[↑ Saspik-meshrover](../README.md)

# ⧫ Аппаратная схема UWB-узла (Stage 0)

#### Схема и питание UWB-узла на ESP32-S3 + DWM3000

Пин-аут сверен с `software/include/pins.h` (SPI2_HOST, GPIO10–15).
Стенд Stage 1 — два идентичных узла с USB-питанием.
...
> Пин-аут сверен с `software/include/pins.h` (SPI2_HOST, GPIO10–15). DWM3000 работает в SPI Mode 0, тактовая частота до 23 МГц — текущие 20 МГц в `config.h` корректны.
...
- Узел собран по односторонней плате с керамической антенной DWM3000; для дальности >20 м может понадобиться выносная антенна (референсная плата Qorvo).
- Прошивка `lib/dwm3000` написана под DW3110 и почищена от DW1000-регистров; все устаревшие упоминания DW1000 схемно неактуальны.