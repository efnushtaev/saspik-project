...
#### 3.1 Юнит по ID
- `GET /api/v1/units/:id`
- Ответ: `{ "unit": { "id", "name", "description?", "objects": [...], "rules": [...] } }` — структура как у элемента `units` в `GET /units/list`. 404, если юнит не найден.

#### 3.2 Создание юнита
- `POST /api/v1/units`
- Тело: `{ "id", "name", "description?" }`
- Созданный юнит не содержит объектов и правил.
- Ответ: `{ "unit": { ... } }`. 400 при невалидных данных (`id`/`name` обязательны) или дубликате `id`.

#### 3.3 Обновление юнита
- `PATCH /api/v1/units/:id`
- Тело: `{ "name", "description?" }` (без `id` — `id` берётся из URL и не изменяем).
- Ответ: `{ "unit": { ... } }`. 404, если юнит не найден; 400 при невалидных данных.

#### 3.4 Удаление юнита
- `DELETE /api/v1/units/:id`
- Удаляет юнит **вместе с его объектами** (каскадно, коллекция `objects`). Правила не затрагиваются.
- Ответ: `{ "success": true }`. 404, если юнит не найден.
...
#### 7.2 Обновление объекта
- `PATCH /api/v1/objects/:id`
- Тело: `{ "name", "type", "spec", "description?", "unitId" }` (без `id` и `topic` — `id` берётся из URL, топик пересчитывается по паттерну `` `${type}/${unitId}/${id}` ``)
- `spec` — массив записей: `{ "key", "model", "unit"? , "minorPart"? }`
- Ответ: `{ "object": { ... } }`. 404, если объект не найден; 400 при невалидных данных.

#### 7.3 Удаление объекта
- `DELETE /api/v1/objects/:id`
- Тело: `{ "unitId" }`
- Ответ: `{ "success": true }`. 404, если объект не найден.
